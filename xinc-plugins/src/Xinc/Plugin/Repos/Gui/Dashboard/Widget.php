...
class Xinc_Plugin_Repos_Gui_Dashboard_Widget extends Xinc_Gui_Widget_Abstract
{
        ...
        switch ($eventId) {
            case Xinc_Gui_Event::PAGE_LOAD: 
                ...
                if (preg_match('/\/dashboard\/projects.*/', $query)) {
                    include_once Xinc_Data_Repository::getInstance()->getWeb(
                        'templates' . DIRECTORY_SEPARATOR . 'dashboard' . DIRECTORY_SEPARATOR . 'projects.phtml'
                    );
                }
                break;
            default:
    ...
    {
        if ($extensionPoint == 'PROJECT_MENU_ITEM' && $this->projectMenuItem !== null) {
            $this->projectMenuItem->registerSubExtension($extension);
        } else {
            parent::registerExtension($extensionPoint, $extension);
        }
    }
